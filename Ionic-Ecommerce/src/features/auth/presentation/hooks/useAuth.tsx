  ...
  return function AuthProvider({ children }: AuthProviderProps) {
    ...
    useEffect(() => {
      ...
      const loadCurrentUser = async () => {
        try {
          const user = await useCases.getLocalCurrentUserUseCase();
    ...
    const login = async (params: LoginParams) => {
      try {
        ...
        const user = await useCases.postLocalLoginUseCase(params);
        setSession({
          user,
    ...
    const register = async (params: RegisterUserParams) => {
      try {
        setIsSubmitting(true);
        setErrorMessage(null);
        const user = await useCases.postLocalRegisterUserUseCase(params);
        setSession({
          user,
          isAuthenticated: true,
        });
    ...
    const logout = async () => {
      try {
        setIsSubmitting(true);
        setErrorMessage(null);
        await useCases.deleteLocalAuthSessionUseCase();
        setSession({
          user: null,
          isAuthenticated: false,
        });
    ...
    const addAddress = async (params: AddAddressParams) => {
      try {
        setIsSubmitting(true);
        setErrorMessage(null);
        const updatedUser = await useCases.postLocalAddressUseCase(params);
    ...
    const setDefaultAddress = async (addressId: string) => {
      try {
        setIsSubmitting(true);
        setErrorMessage(null);
        const updatedUser = await useCases.patchLocalDefaultAddressUseCase(addressId);
    ...
    const registerOrder = async (params: RegisterOrderParams) => {
      try {
        setErrorMessage(null);
        const updatedUser = await useCases.postLocalRegisterOrderUseCase(params);
