...
export function createUseAdminCatalog(useCases: AdminCatalogUseCasesProtocol) {
  return function useAdminCatalog() {
      ...
      const loadAdminCatalog = async () => {
        try {
          const [options, loadedProducts] = await Promise.all([
            useCases.getAdminCatalogOptionsUseCase(),
            useCases.getAdminProductsUseCase(),
          ]);
    ...
    const submitProduct = async () => {
      try {
        ...
        const savedProduct = selectedProductId
          ? await useCases.putAdminProductUseCase(selectedProductId, productPayload)
          : await useCases.postAdminProductUseCase(productPayload);
    ...
    const removeProduct = async (productId: string) => {
      try {
        ...
        await useCases.deleteAdminProductUseCase(productId);
        setProducts((currentProducts) =>
